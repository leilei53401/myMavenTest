		...
		for (String processFile : files) {
			...
			try {
				...
				while ((line = br.readLine()) != null) {
					count++;
					String param = StringUtils.substringAfter(line, "loggerAdStRequestInfo - ");
					Map<String, String> lineMap = lineMap(param);
					if(lineMap == null){
						System.out.println("播放串["+line+"]解析异常,lineMap==null!");
						continue;
					}
					
					String ven = lineMap.get("ven");
					String venName = venMap.get(ven);
					String amid = lineMap.get("amid");
