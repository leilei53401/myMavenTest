...
public class MemProcessNumberFile {
	...
	public static void main(String[] args) {
		...
		for (String processFile : files) {
			...
			try {
				...
				String line = null;
				// 写文件
				String outFile =  processFile.replaceAll(".log", ".out");
				...
				while ((line = br.readLine()) != null) {
					count++;
					String processName = StringUtils.substringBetween(line,"[","]");
					
					if(!processName.startsWith("TaskExePool")){
						continue;
					}
					
					String value = StringUtils.substringAfter(processName, "TaskProcessor");
					int intValue =	Integer.valueOf(value);
						if(max<intValue){
							max = intValue; 
					}
					
					bw1.write(processName);
					bw1.newLine();
				}
