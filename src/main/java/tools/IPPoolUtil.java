...
/**
 * @author shaoyl Oct 31, 2013
 * 
 */
public class IPPoolUtil {  
	   public static void main(String[] args)
	   {  
	/*       System.out.println(getNetMask("255.255.240.0"));  
	         
	       System.out.println(getPoolMax(getNetMask("255.255.255.128")));  */
	         
	       System.out.println(getEndIP("116.58.220.0",2).getStartIP());  //192.168.16.129
	         
	       System.out.println(getEndIP("116.58.220.0",2).getEndIP());  //192.168.16.254
	   }  
	   /** 
	    * 根据起始IP地址和子网掩码计算终止IP 
	    */  
	   ...
	   {  
	       ...
	       //nets.setStartIP(start[0]+"."+start[1]+"."+start[2]+"."+(Integer.valueOf(start[3])+1));//+1为第�?��可用IP  
	       nets.setStartIP(start[0]+"."+start[1]+"."+start[2]+"."+(Integer.valueOf(start[3])));//�?1，第�?��为网络IP
...
//	        return rets[0]+"."+rets[1]+"."+rets[2]+"."+(rets[3]-1); //-1为去掉广播IP
	        return rets[0]+"."+rets[1]+"."+rets[2]+"."+(rets[3]);  //�?1�?��为广播IP  
	    }  
	    /** 
	     * 计算子网大小 
	     */  
